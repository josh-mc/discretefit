...
#'
#' \item{statistic}{the value of the root-mean-square test statistic}
...
rms_gof <- function(x,
  ...
  out <- simulate_p(1, x, p, reps, tolerance)

  names(out$statistic) <- "RMS"

  val <- list(p.value = out$p_value,
              statistic = out$statistic,
              method = "Simulated root-mean-square goodness-of-fit test",
